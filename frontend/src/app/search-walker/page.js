...
export default function SearchWalker() {
  ...
  const getDuration = (startTime, arriveTime) => {
    ...
    // 计算两个日期对象的时间差，单位是毫秒
    const timeDifference = arriveDate - startDate;

    // 将毫秒转换为小时和分钟
    const hours = Math.floor(timeDifference / (1000 * 60 * 60)); // 毫秒 -> 小时
    const minutes = Math.floor((timeDifference % (1000 * 60 * 60)) / (1000 * 60)); // 毫秒 -> 分钟
              ...
              requests.map((request) => (
                  <div key={request.id}
                    ...
                    <div>
                      ...
                      <p><strong>Duration:</strong> {getDuration(request.startTime, request.arriveTime)}</p>
                    </div>
